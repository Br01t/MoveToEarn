...
const Dashboard: React.FC<DashboardProps> = ({ user, zones, onSyncRun, onClaim, onBoost, onDefend }) => {
  ...
  const earningRate = zones.filter((z) => z.ownerId === user.id).reduce((acc, z) => acc + 0.5 * z.interestRate, 0) * 6; // Per minute estimate (since loop is 10s)
  ...
  const handleSyncSubmit = (e: React.FormEvent) => {
    ...
    if (!proofPreview) {
      if (!window.confirm("Warning: Submitting without a screenshot reduces trust score. Continue?")) {
        return;
      }
    } else {
    ...
    <div className="relative w-full h-[85vh] overflow-hidden bg-gray-900 shadow-inner">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors ${
              filterMode === "ALL" ? "bg-gray-600 text-white" : "bg-gray-900/50 text-gray-400 hover:bg-gray-700"
            }`}
          >
            ALL
          </button>
          <button
            ...
            className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors ${
              filterMode === "MINE" ? "bg-emerald-600 text-white" : "bg-gray-900/50 text-gray-400 hover:bg-emerald-900/30"
            }`}
          >
            MINE
          </button>
          <button
            ...
            className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors ${
              filterMode === "ENEMY" ? "bg-red-600 text-white" : "bg-gray-900/50 text-gray-400 hover:bg-red-900/30"
            }`}
          >
            ENEMY
          </button>
      ...
      <div className="absolute top-4 right-4 z-20 flex flex-col gap-2">
        <div className="bg-gray-800/90 backdrop-blur p-2 rounded-lg border border-gray-700 text-xs text-white flex flex-col gap-1 mb-2">
          <span className="flex items-center gap-2">
            <div className="w-3 h-3 bg-emerald-400 rounded-sm shadow shadow-emerald-500/50"></div> My Zones
          </span>
          <span className="flex items-center gap-2">
            <div className="w-3 h-3 bg-red-400 rounded-sm shadow shadow-red-500/50"></div> Enemy
          </span>
          <span className="flex items-center gap-2">
            <div className="w-3 h-3 bg-amber-400 rounded-sm shadow shadow-amber-500/50"></div> Boosted
          </span>
          <span className="flex items-center gap-2">
            <div className="w-3 h-3 bg-cyan-400 rounded-sm shadow shadow-cyan-500/50"></div> Shielded
          </span>
        </div>

        <button onClick={zoomIn} className="p-2 bg-gray-800 hover:bg-gray-700 text-white rounded-lg border border-gray-600 shadow-lg">
          <ZoomIn size={20} />
        </button>
        <button onClick={zoomOut} className="p-2 bg-gray-800 hover:bg-gray-700 text-white rounded-lg border border-gray-600 shadow-lg">
          <ZoomOut size={20} />
        </button>
                  ...
                  <foreignObject x={-HEX_SIZE} y={-HEX_SIZE} width={HEX_SIZE * 2} height={HEX_SIZE * 2} pointerEvents="none">
                    <div className="w-full h-full flex flex-col items-center justify-center text-center p-2 leading-none pointer-events-none relative gap-2">
                      ...
                      <div className="flex items-center justify-center w-full">
                        <span
                          className="text-xs sm:text-sm font-black text-white tracking-wider drop-shadow-lg leading-tight whitespace-normal break-words max-w-[150px] transition-transform duration-300 group-hover:scale-105"
                      ...
                      <div className="flex flex-col items-center justify-center w-full">
                        <div className="flex justify-center w-full mb-2">
                          <span
                            className={`text-lg font-bold text-white drop-shadow-md px-2 py-0.5 rounded-full border border-white/20 transition-colors duration-300 ${
                              zone.ownerId === user.id
                                ? boosted
                                  ? "bg-amber-600/90 border-amber-400"
                                  : shielded
                                  ? "bg-cyan-800/90 border-cyan-500"
                                  : "bg-emerald-900/80"
                                : "bg-red-900/80"
                            }`}
                          >
      ...
      {selectedZone && (
        <div className="absolute bottom-24 right-6 w-80 bg-gray-900/95 backdrop-blur-md rounded-2xl border border-emerald-500/30 shadow-[0_0_30px_rgba(0,0,0,0.5)] overflow-hidden animate-slide-up z-30">
          <div className="relative p-5">
            <button onClick={() => setSelectedZone(null)} className="absolute top-4 right-4 text-gray-400 hover:text-white">
              <X size={20} />
            </button>
                  ...
                  <div className="flex gap-2">
                    ...
                    {!isBoostActive(selectedZone) ? (
                      <button
                        ...
                        className={`flex-1 py-3 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all border ${
                          boostItem
                            ? "bg-amber-600 hover:bg-amber-500 border-amber-400/20 shadow-lg"
                    ...
                    {!isShieldActive(selectedZone) ? (
                      <button
                        ...
                        className={`flex-1 py-3 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all border ${
                          defenseItem
                            ? "bg-cyan-600 hover:bg-cyan-500 border-cyan-400/20 shadow-lg"
                            : "bg-gray-800 text-gray-500 border-gray-700 cursor-not-allowed"
                        }`}
      ...
      {showSyncModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-gray-800 rounded-2xl border border-gray-700 w-full max-w-md shadow-2xl overflow-hidden animate-slide-up">
            ...
            <form onSubmit={handleSyncSubmit} className="p-6 space-y-6">
              <div className="space-y-4">
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-2">If this zone exists, you will reinforce/contest it. If it's new, you can mint it.</p>
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Proof of Run (Strava/Garmin/...)</label>
                  <p className="text-xs text-gray-500 mt-2 mb-4">It will be automatically validated.</p>
                  <input type="file" accept="image/*" hidden ref={fileInputRef} onChange={handleFileSelect} />

                  {!proofPreview ? (
                    <div
                      onClick={() => fileInputRef.current?.click()}
                      className="w-full border-2 border-dashed border-gray-600 rounded-lg p-6 flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500 hover:bg-gray-800/50 transition-colors group"
                    >
                      <ImageIcon className="text-gray-500 group-hover:text-emerald-400 mb-2" size={32} />
                      <span className="text-sm text-gray-400 group-hover:text-white">Click to upload screenshot</span>
                      <span className="text-xs text-gray-600 mt-1">Supports JPG, PNG</span>
                    </div>
                  ) : (
                    <div className="relative w-full rounded-lg overflow-hidden border border-gray-600 group">
                      <img src={proofPreview} alt="Run Proof" className="w-full h-40 object-cover opacity-80" />
                      <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          type="button"
                          onClick={() => setProofPreview(null)}
                          className="bg-red-500/80 hover:bg-red-500 text-white p-2 rounded-full flex items-center gap-2 px-4"
                        >

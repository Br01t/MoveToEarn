...
import { Play, ZoomIn, ZoomOut, Move, X, UploadCloud, MapPin, CheckCircle, Zap, Search, ShoppingBag, Clock, Shield, Globe, Image as ImageIcon, Trash2, FileText, Crown, Loader, AlertTriangle, Lock, Filter, ChevronDown, ChevronUp, HelpCircle } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ user, zones, onSyncRun, onClaim, onBoost, onDefend, onNavigate }) => {
  ...
  return (
    <div className="relative w-full h-[calc(100vh-64px)] md:h-[calc(100vh-64px)] overflow-hidden bg-gray-900 shadow-inner">
      ...
      {/* SEARCH AND FILTER - Collapsible on Mobile */}
      <div className={`absolute top-14 left-2 z-20 flex flex-col items-start transition-all duration-300 ${isFilterOpen ? 'w-64' : 'w-10'}`}>
          <button 
             onClick={() => setIsFilterOpen(!isFilterOpen)} 
             className="w-10 h-10 bg-gray-800/90 backdrop-blur-md rounded-full border border-gray-700 shadow-lg flex items-center justify-center text-white hover:text-emerald-400 z-30"
          >
